import { FullSchedule, WeekSchedule } from '../../types/schedule';

const STORAGE_KEY = 'doctor_schedules_v1';

function seedSchedules(): Record<string, FullSchedule> {
  const pick = (floor: number, idx: number) => String(floor * 100 + idx);
  const make = (id: string, name: string, dept: string, defs: WeekSchedule): FullSchedule => ({
    doctorId: id,
    doctorName: name,
...
export function loadSchedules(): Record<string, FullSchedule> {
  try {
...
export function saveSchedules(data: Record<string, FullSchedule>) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
}

export function upsertDoctorSchedule(doctorId: string, schedule: FullSchedule): FullSchedule {
...
export function deleteDoctorSchedule(doctorId: string) {
  const all = loadSchedules();
  delete all[doctorId];
  saveSchedules(all);
}
