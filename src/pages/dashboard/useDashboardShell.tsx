...
export function useDashboardShell() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // is dynamic here.
        // We must attempt the API call if a floor is selected, even if we don't have
        // a specific floorId (the API function will use a default or handle it).
        // If the API returns empty/204/404, we'll fall back to mock data below.
        if (!selectedBuilding || typeof selectedFloor !== 'number') {
  ...
  const enrichedSelectedBuilding = useMemo(() => {
    if (!selectedBuilding) return null;
    // 1. Try to find in the current campus list (most likely)
    const match = buildingsForCampus.find((b) => b.id === selectedBuilding.id);
    if (match) return match;

    // 2. Fallback: try to look up in hierarchy directly if keys match
    // (covers edge case where we might have a selectedBuilding but crumbs differ?)
    if (hierarchyByCity && selectedBuilding.city && selectedBuilding.campus) {
      const city = hierarchyByCity.get(selectedBuilding.city);
      const campus = city?.campuses.get(selectedBuilding.campus);
      const bKey = String(selectedBuilding.name).toLowerCase().trim();
      const raw = campus?.buildings.get(bKey);
      if (raw) return buildBuildingFromHierarchy(raw);
    }
    return selectedBuilding;
  }, [selectedBuilding, buildingsForCampus, hierarchyByCity, buildBuildingFromHierarchy]);
  ...
  return {
    ...
    selectedCity,
    selectedCampus,
    selectedBuilding: enrichedSelectedBuilding,
    selectedFloor,
    selectedFloorId,
    floorView,
    zone,
